...
if __name__ == '__main__':  #
    ...
    for selectedCell in range(10):
        for d in patch_diameter:
            tuning_curve = np.zeros([len(w_rc_ex), len(k_id)])

            for i, w in enumerate(w_rc_ex):
                network = create_spatial_network_impulse(cells[selectedCell], nt=nt, nr=nr, dt=dt, dr=dr,
        ...
        for ax, w_rc, curves, title, label in zip([ax1, ax2, ax3],
            ...
            for w, curve in zip(w_rc[:], curves[:]):
                ax.plot(wavenumber, curve/0.1, '-', label=r"${} \times$".format(round(w, 1)) + label)
        ...
        for ax, w_rc, curves, title, label in zip([ax4, ax5, ax6],
            ...
            for w, curve in zip(w_rc[:], curves[:]):
                ax.plot(wavenumber, curve/0.35, '-', label=r"${} \times$".format(round(w, 1)) + label)
        ...
        #ax8.plot(cells[selectedCell].sf_X[0], cells[selectedCell].sf_Y[0])
        #ax9.plot(cells[selectedCell].sf_X[0], cells[selectedCell].sf_Y[0])
        #ax7.plot(cells[selectedCell].sf_X[0], cells[selectedCell].sf_Y[0])
        #ax7.set_xscale("log")
        #ax7.grid(which="minor", lw=1.5)
        #ax7.grid(which="major", lw=1.5)
        ...
        #ax7.set_ylabel("Response")
        #ax7.set_xlabel("Wavenumber (1/deg)")

        #fig.tight_layout()


        fig2, (axa, axb) = plt.subplots(1, 2, figsize=(7, 5), dpi=100)
        axa.plot(cells[selectedCell].sf_X[0], NormalizeData(cells[selectedCell].sf_Y[0]))
        axa.set_xscale("log")
